...
def merge_sets_until_done(g, start_regions, regions_left=4):
    ...
    # reduce active to some n good active points (rather than making all the edge points active)
    # what is a "good" starting point
    # we also want to delay as much as possible when the points will merge up as if they all merge up in the beginning we are left with 1 large blob and like 3 individual superpixels
    # what we could do instead is seed active superpixels everywhere???

    # naw nothing is robust compared to just having every edge be active

    # maybe try:
    # - without regularization factor
    # making every 2nd/3rd edge a starting point
    ...
    while len(ds.subsets()) > regions_left:
        ...
        for n1 in active:
            for n2 in nx.all_neighbors(g, n1):
                ...
                # reduce loss of spixel count if we the edge is within an object
                # if g[n1][n2]['within object']:
                #     dst /= 5
